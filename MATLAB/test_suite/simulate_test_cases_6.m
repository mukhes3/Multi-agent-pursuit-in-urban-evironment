...
% File Parameters
TEST = 5;
filename_prefix = sprintf('test%d_evader_time', TEST);
filename = 'desktop';%[filename_prefix '__' datestr(now,'yyyy-mm-dd__HH-MM-SS')];
...
nRuns = length(vAlgs) * length(vNPursuers) * length(vEvaderTime) * nTrials;
results(nRuns).trial = nTrials;
lastrun = '';

run = 0;
for alg = vAlgs;
for evader_time = vEvaderTime
...
lastrun = sprintf('test%d_run_%d_of_%d', TEST, run, nRuns);
f = fopen(lastrun, 'w+');
fclose(f);
